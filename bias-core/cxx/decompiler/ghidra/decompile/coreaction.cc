...

{
  ...
  if (mode < 2)
    ...
    if (defaultSize != 4)
      defaultSize = 8;
    checkLanes.addLaneSize(defaultSize);               // Default lane size
  }
...
int4 ActionCallbackNameVars::apply(Funcdata &data)

{
  data.getScopeLocal()->applyNamesViaCallback(data);
  return 0;
}
...
int4 ActionDeEndbrAnnot::apply(Funcdata &data)

{
  UserPcodeOp *endbr_op = data.getArch()->userops.getOp("endbr");
  ...
  while (iter != enditer) {
    PcodeOp *op = *iter++;
    Varnode *vnd = op->getIn(0);
    if (vnd == nullptr) {
      continue;
    }
    if (vnd->getOffset() == endbr_op->getIndex()) {
      data.opDestroy(op);
    }
  }
...
int4 ActionDePacAnnot::apply(Funcdata &data)

{
  if (!indices.has_value()) {
    indices.emplace();
    for (const char *name : intrinsics) {
      UserPcodeOp *op = data.getArch()->userops.getOp(name);
      if (op != nullptr) {
        indices->insert(op->getIndex());
      }
    }
  }
  ...
  while (iter != enditer) {
    ...
    if (indices->find(vnd->getOffset()) != indices->end()) {
      data.opDestroy(op);
    }
  }
...
int4 ActionDePointerAnnot::apply(Funcdata &data)

{
  UserPcodeOp *ptr_op = data.getArch()->userops.getOp("pointer");
  ...
  while (iter != enditer) {
    ...
    if (vnd->getOffset() == ptr_op->getIndex()) {
      data.opDestroy(op);
    }
  }
  ...
  for(iter=data.beginLoc();iter!=data.endLoc();++iter) {
    ...
    if (entry != (SymbolEntry *)0 && !vn->isTypeLock() && entry->getSymbol()->isTypeLocked()) {
      ...
      if (ct == (Datatype *)0 || ct->getMetatype() == TYPE_UNKNOWN) { // If we can't resolve, or resolve to UNKNOWN
        Address usep = vn->isWritten() ? vn->getUsePoint(data) : data.getAddress();
        Datatype *ctr = data.getScopeLocal()->queryDataType(usep, vn);
	ct = ctr == (Datatype *)0 ? vn->getLocalType(needsBlock) : ctr;		// Let data-type float, even though parent symbol is type-locked
      }
    }
    else {
      Address usep = vn->isWritten() ? vn->getUsePoint(data) : data.getAddress();
      Datatype *ctr = data.getScopeLocal()->queryDataType(usep, vn);
      ct = ctr == (Datatype *)0 ? vn->getLocalType(needsBlock) : ctr;
    }
    if (needsBlock)
...

{
  ...
  act->addAction( new ActionDeEndbrAnnot("base"));
  act->addAction( new ActionDePacAnnot("base"));
  act->addAction( new ActionDePointerAnnot("base"));
  ...
  act->addAction( new ActionCallbackNameVars("merge") );
