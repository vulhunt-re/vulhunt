...
static inline hashword hash_mixin(hashword val1,hashword val2)
...
public:
  using iterator = vector<Signature *>::iterator;
  using const_iterator = vector<Signature *>::const_iterator;
  ...
  iterator begin() { return sigs.begin(); }
  iterator end() { return sigs.end(); }
  const_iterator begin() const { return sigs.begin(); }
  const_iterator end() const { return sigs.end(); }
};
...
public:
  ...
  GraphSigManager(uint4 settings);				///< Constructor
