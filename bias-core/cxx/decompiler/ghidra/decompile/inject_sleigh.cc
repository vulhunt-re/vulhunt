...
InjectPayloadCallfixup::InjectPayloadCallfixup(const string &sourceName, const string &name)
  : InjectPayloadSleigh(sourceName,name,CALLFIXUP_TYPE)
{
}
...

{
  ...
  if (type == InjectPayload::CALLFIXUP_TYPE)
    injection.push_back(new InjectPayloadCallfixup(sourceName,name));
