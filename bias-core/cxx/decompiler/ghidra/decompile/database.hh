...
public:
  ...
  bool isInvalid(void) const { return symbol == nullptr || (addr.isInvalid() && (hash==0)); }	///< Is \b this storage \e invalid
...
inline uint4 SymbolEntry::getAllFlags(void) const {
  return extraflags | (symbol ? symbol->getFlags() : 0);
}

inline bool SymbolEntry::isAddrTied(void) const {
  return symbol != nullptr && ((symbol->getFlags()&Varnode::addrtied)!=0);
}
...
protected:
  ...
  static const Scope *stackCodeLabel(const Scope *scope1,
					  ...
					  const Address &addr,
					  LabSymbol **addrmatch);
  static const Scope *stackDataType(const Scope *scope1,
				  const Scope *scope2,
				  const Address &addr,
                                  const Varnode *vnd,
				  Datatype **datatype);
...
public:
  ...
  virtual Datatype *findDataType(const Address &addr, const Varnode* vnd) const=0;
  ...
  virtual void applyNamesViaCallback(Funcdata &data) const;
  ...
  Datatype *queryDataType(const Address &addr, const Varnode *vnd) const;
...
public:
  ...
  virtual Datatype *findDataType(const Address &addr, const Varnode *vnd) const;
