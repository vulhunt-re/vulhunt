  ...
  while(i<field.size()) {
    ...
    if (subtype->getMetatype() == TYPE_ARRAY) {
      ...
      *elSize = ((TypeArray *)subtype)->getBase()->getAlignSize();
      ...
      int8 suboff;
      Datatype *res =
          subtype->nearestArrayedComponentForward(remain, &suboff, elSize);
      if (res != (Datatype *)0) {
...
Datatype *TypeFactory::getTypedef(Datatype *ct,const string &name,uint8 id,uint4 format,bool rebuild)
...

{
  ...
  // NOTE(slt): this modification matches 21a6a276b2ceb91f31dc2d1f2b4f03d795974107
