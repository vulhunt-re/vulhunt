...
/// \brief Choose names for all high-level variables via a callback (HighVariables)
class ActionCallbackNameVars : public Action {
public:
  ActionCallbackNameVars(const string &g) : Action(rule_onceperfunc,"namevars_callback",g) {}	///< Constructor
  virtual Action *clone(const ActionGroupList &grouplist) const {
    if (!grouplist.contains(getGroup())) return (Action *)0;
    return new ActionCallbackNameVars(getGroup());
  }
  virtual int4 apply(Funcdata &data);
};
...
class ActionDeEndbrAnnot : public Action {
public:
  ActionDeEndbrAnnot(const string &g) : Action(0,"deendbrannot",g) {}	///< Constructor
  virtual Action *clone(const ActionGroupList &grouplist) const {
    if (!grouplist.contains(getGroup())) return (Action *)0;
    return new ActionDeEndbrAnnot(getGroup());
  }
  virtual int4 apply(Funcdata &data);
};

class ActionDePacAnnot : public Action {
public:
  ActionDePacAnnot(const string &g) : Action(0,"depacannot",g) {}	///< Constructor
  virtual Action *clone(const ActionGroupList &grouplist) const {
    if (!grouplist.contains(getGroup())) return (Action *)0;
    return new ActionDePacAnnot(getGroup());
  }
  virtual int4 apply(Funcdata &data);

private:
  inline static const std::array<const char *, 13> intrinsics = {
      "pacda", "pacdb",  "pacdza", "pacdzb", "pacga",  "pacia", "paciza",
      "pacib", "pacizb", "AuthIA", "AuthIB", "AuthDA", "AuthDB"};
  std::optional<std::unordered_set<int4>> indices;
};

class ActionDePointerAnnot : public Action {
public:
  ActionDePointerAnnot(const string &g) : Action(0,"depointerannot",g) {}	///< Constructor
  virtual Action *clone(const ActionGroupList &grouplist) const {
    if (!grouplist.contains(getGroup())) return (Action *)0;
    return new ActionDePointerAnnot(getGroup());
  }
  virtual int4 apply(Funcdata &data);
};
