...
{
  std::lock_guard<std::mutex> lock(DocumentStorage::mutex);
  std::pair<map<string, Document *>::iterator, bool> ret = DocumentStorage::cache.insert(map<string, Document *>::value_type(filename, nullptr));
