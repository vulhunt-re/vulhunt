...
{
  // NOTE: we use sizeof(uintp) here to get the size of a pointer, which is stuffed
  // into the Varnode as an address.
  Datatype *ct = glb->types->getBase(sizeof(uintp),TYPE_UNKNOWN);
  AddrSpace *cspc = glb->getIopSpace();
  Varnode *vn = vbank.create(sizeof(uintp),Address(cspc,(uintb)(uintp)op),ct);
  assignHigh(vn);
  return vn;
}
...

{
  ...
  Varnode *vn = vbank.create(sizeof(uintp),glb->createConstFromSpace(spc),ct);
  assignHigh(vn);
  return vn;
}
...

{
  ...
  Varnode *vn = vbank.create(sizeof(uintp),Address(cspc,(uintb)(uintp)fc),ct);
  assignHigh(vn);
  return vn;
}
