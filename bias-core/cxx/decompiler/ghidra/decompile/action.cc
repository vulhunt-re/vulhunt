...
void Action::resetTimeout(const std::optional<std::chrono::milliseconds> t)
{
  timeout = t;
  ...
  start_time = std::chrono::duration_cast<std::chrono::milliseconds>(duration);
}

void Action::resetTimeout(const std::optional<std::chrono::milliseconds> t, const std::chrono::milliseconds st)
{
  timeout = t;
  start_time = st;
}
...

{
  ...
  do {
    if (timeout) {
      auto now = std::chrono::system_clock::now();
      auto duration = now.time_since_epoch();
      auto current_time = std::chrono::duration_cast<std::chrono::milliseconds>(duration);

      if (current_time - start_time >= *timeout) {
        throw LowlevelError("decompilation timed out");
      }
    }
...
void ActionGroup::resetTimeout(const std::optional<std::chrono::milliseconds> t)
{
  vector<Action *>::iterator iter;

  Action::resetTimeout(t); // propagate down
  for(iter=list.begin();iter!=list.end();++iter)
    (*iter)->resetTimeout(t, start_time);
}

void ActionGroup::resetTimeout(const std::optional<std::chrono::milliseconds> t, const std::chrono::milliseconds st)
{
  vector<Action *>::iterator iter;

  Action::resetTimeout(t, st);
  for(iter=list.begin();iter!=list.end();++iter)
    (*iter)->resetTimeout(t, st);
}
...
void ActionPool::resetTimeout(const std::optional<std::chrono::milliseconds> t)
{
  Action::resetTimeout(t);
}

void ActionPool::resetTimeout(const std::optional<std::chrono::milliseconds> t, const std::chrono::milliseconds st)
{
  Action::resetTimeout(t, st);
}
