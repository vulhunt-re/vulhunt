...

{
  ...
  for(iter=oplist.begin();iter!=oplist.end();++iter) {
    ...
    Varnode *rvnd = getReplaceVarnode(rout);
    PcodeOp *oldop = rvnd->getDef();

    if (oldop) {
      int4 size = rvnd->getSize();
      if (useSameAddress(rout)) {
        Address addr = getReplacementAddress(rout);
        fd->opSetOutput(oldop, fd->newVarnode(size,addr));
      }
      else
        fd->opSetOutput(oldop, fd->newUnique(size));
    }

    fd->opSetOutput(newop, rvnd);
