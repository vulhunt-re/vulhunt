...

{
  for(;;) {
    ...
    if (ct->getMetatype()==TYPE_PTR)
      ct = const_cast<const Datatype *>(static_cast<Datatype *>(static_cast<TypePointer *>(const_cast<Datatype *>(ct))->getPtrTo()));
    else if (ct->getMetatype()==TYPE_ARRAY)
      ct = const_cast<const Datatype *>(static_cast<Datatype *>(static_cast<TypeArray *>(const_cast<Datatype *>(ct))->getBase()));
    else if (ct->getMetatype()==TYPE_CODE) {
      const FuncProto *proto = static_cast<TypeCode *>(const_cast<Datatype *>(ct))->getPrototype();
      if (proto != (const FuncProto *)0)
	ct = const_cast<const Datatype *>(proto->getOutputType());
      else
	ct = const_cast<const Datatype *>(static_cast<Datatype *>(glb->types->getTypeVoid()));
    }
    else
...

{
  ...
  int4 id0 = emit->beginFunctionCall(op);
