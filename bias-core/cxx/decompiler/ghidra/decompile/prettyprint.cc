...
ElementId ELEM_FUNCTION_CALL = ElementId("funccall",1000);
...
int4 EmitMarkup::beginFunctionCall(const PcodeOp *op) {
  encoder->openElement(ELEM_FUNCTION_CALL);
...
void EmitMarkup::endFunctionCall(int4 id) {
  encoder->closeElement(ELEM_FUNCTION_CALL);
}
...

{
  ...
  case func_e:	// endFunction
    ...
    break;
  case call_b:
    emit->beginFunctionCall(ptr_second.fc);
    break;
  case call_e:
    emit->endFunctionCall(count);
    break;
...
int4 EmitPrettyPrint::beginFunctionCall(const PcodeOp *op)

{
  checkstart();
  TokenSplit &tok( tokqueue.push() );
  int4 id = tok.beginFunctionCall(op);
  scan();
  return id;
}

void EmitPrettyPrint::endFunctionCall(int4 id)

{
  checkend();
  TokenSplit &tok( tokqueue.push() );
  tok.endFunctionCall(id);
  scan();
}
