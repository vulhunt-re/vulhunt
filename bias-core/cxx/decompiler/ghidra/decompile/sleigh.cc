...

{
  ...
  for(int4 i=0;i<minimumreuse;++i) {
    ...
    pos->initialize(120,65,constspace);
    list[i] = pos;
  }
...
uintm Sleigh::getContextVariable(const string &name, const Address &addr) {
  return context_db->getVariable(name, addr);
}

void Sleigh::setContextVariable(const string &name, const Address &addr, uintm val) {
  context_db->setVariable(name, addr, val);
}

void Sleigh::clearCaches() {
  pcode_cache.clear();
  discache->invalidate();
  cache->invalidate();
}
