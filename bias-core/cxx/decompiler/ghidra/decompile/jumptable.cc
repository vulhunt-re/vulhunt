...

{
  ...
  iter = table.begin();
  lastiter = iter;
  Address nextaddr = (*iter).addr + static_cast<intb>((*iter).size) * static_cast<intb>((*iter).num);
  ++iter;
  for(;iter!=table.end();++iter) {
    if (( (*iter).addr == nextaddr ) && ((*iter).size == (*lastiter).size)) {
      (*lastiter).num += (*iter).num;
      nextaddr = (*iter).addr + static_cast<intb>((*iter).size) * static_cast<intb>((*iter).num);
    }
...

{
  ...
  uintb right = (left + static_cast<intb>(step) * static_cast<intb>(nm)) & range.getMask();
