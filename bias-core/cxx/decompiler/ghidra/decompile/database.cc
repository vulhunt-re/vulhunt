...
const Scope *Scope::stackDataType(const Scope *scope1,
				  const Scope *scope2,
				  const Address &addr,
                                  const Varnode *vnd,
				  Datatype **datatype)
{
  Datatype *dt;
  while((scope1 != (const Scope *)0)&&(scope1 != scope2)) {
    dt = scope1->findDataType(addr, vnd);
    if (dt != (Datatype *)0) {
      *datatype = dt;
      return scope1;
    }
...
Datatype *Scope::queryDataType(const Address &addr, const Varnode *vnd) const

{
  Datatype *res = (Datatype *)0;
  const Scope *basescope = glb->symboltab->mapScope(this,addr,addr);
  stackDataType(basescope, (const Scope *)0, addr, vnd, &res);
  return res;
}
...
void Scope::applyNamesViaCallback(Funcdata &data) const

{
  // apply up and up
  const Scope *scope = static_cast<const Scope *>(this);
  if ((scope = scope->getParent()) != nullptr) {
    scope->applyNamesViaCallback(data);
  }
}
						   ...
						   const Address &usepoint) const
{
  ...
  if (rangemap != (EntryMap *)0) {
    ...
    while(res.first != res.second) {
      --res.second;
      SymbolEntry *entry = &(*res.second);
      if (entry->isInvalid()) { // seems like we can insert a nullptr symbol?
        continue;
      }
      if (entry->getLast() >= end && entry->getFirst() <= end) { // We contain the range
...
Datatype *ScopeInternal::findDataType(const Address &addr, const Varnode *vnd) const
{
  return (Datatype *)0;
}
