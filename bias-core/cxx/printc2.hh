...
/// \brief Factory and static initializer for the "c-language" back-end to the
/// decompiler
///
/// The singleton adds itself to the list of possible back-end languages for the
/// decompiler and it acts as a factory for producing the PrintC object for
/// emitting c-language tokens.
class BrlyCCapability : public PrintLanguageCapability {
  static BrlyCCapability printCCapability;     ///< The singleton instance
  BrlyCCapability(void);                       ///< Initialize the singleton
  BrlyCCapability(const BrlyCCapability &op2); ///< Not implemented
  BrlyCCapability &operator=(const BrlyCCapability &op); ///< Not implemented
public:
...
struct BrlyPartialSymbolEntry {
  ...
  EmitMarkup::syntax_highlight
      hilite; ///< Highlight information for the field token
};
...
///  - loop structures
///  - etc.
class BrlyC : public PrintLanguage {
protected:
  ...
  static OpToken
      greater_equal;        ///< The \e greater \e than \e or \e equal operator
  ...
  static OpToken type_expr_space;   ///< Type declaration involving a space
                                    ///< (identifier or adornment)
  ...
  static OpToken
      enum_cat; ///< The \e concatenation operator for enumerated values
public:
  ...
  static const std::string
      typePointerRelToken; ///< The token to print indicating PTRSUB relative to
                           ///< a TypePointerRel
protected:
  bool option_NULL;        ///< Set to \b true if we should emit NULL keyword
  bool option_inplace_ops; ///< Set to \b true if we should use '+=' '&=' etc.
  bool option_convention;  ///< Set to \b true if we should print calling
                           ///< convention
  bool option_nocasts;     ///< Don't print a cast if \b true
  bool option_unplaced;    ///< Set to \b true if we should display unplaced
                           ///< comments
  bool option_hide_exts; ///< Set to \b true if we should hide implied extension
                         ///< operations
  std::string nullToken; ///< Token to use for 'null'
  std::string
      sizeSuffix; ///< Characters to print to indicate a \e long integer token
  CommentSorter
      commsorter; ///< Container/organizer for comments in the current function
  ...
  void pushSymbolScope(
      const Symbol *symbol); ///< Push tokens resolving a symbol's scope
  void emitSymbolScope(
      const Symbol *symbol); ///< Emit tokens resolving a symbol's scope
  virtual void
  pushTypeStart(const Datatype *ct,
                bool noident); ///< Push part of a data-type declaration onto
                               ///< the RPN stack, up to the identifier
  virtual void
  pushTypeEnd(const Datatype *ct); ///< Push the tail ends of a data-type
                                   ///< declaration onto the RPN stack
  ...
  void pushEnumConstant(uintb val, const TypeEnum *ct, tagtype tag, const Varnode *vn,
                        const PcodeOp *op);
  virtual bool pushPtrCharConstant(uintb val, const TypePointer *ct,
  ...
  bool checkArrayDeref(
      const Varnode *vn) const; ///< Determine whether a LOAD/STORE expression
                                ///< requires pointer '*' syntax
  void emitStructDefinition(const TypeStruct *ct); ///< Emit the definition of a
                                                   ///< \e structure data-type
  void emitEnumDefinition(const TypeEnum *ct); ///< Emit the definition of an \e
                                               ///< enumeration data-type
  void emitPrototypeOutput(const FuncProto *proto,
                           const Funcdata *fd); ///< Emit the output data-type
                                                ///< of a function prototype
  void emitPrototypeInputs(
      const FuncProto
          *proto); ///< Emit the input data-types of a function prototype
  void emitGlobalVarDeclsRecursive(
      Scope *symScope); ///< Emit variable declarations for all global symbols
                        ///< under given scope
  void emitLocalVarDecls(
      const Funcdata *fd); ///< Emit variable declarations for a function
  void emitStatement(
      const PcodeOp *inst); ///< Emit a statement in the body of a function
  bool emitInplaceOp(const PcodeOp *op); ///< Attempt to emit an expression
                                         ///< rooted at an \e in-place operator
  void emitGotoStatement(const FlowBlock *bl, const FlowBlock *exp_bl,
                         uint4 type);
  void emitSwitchCase(
      int4 casenum,
      const BlockSwitch *switchbl);    ///< Emit labels for a \e case block
  void emitLabel(const FlowBlock *bl); ///< Emit a formal label for a given
                                       ///< control-flow block
  void emitLabelStatement(
      const FlowBlock
          *bl); ///< Emit any required label statement for a given basic block
  void emitAnyLabelStatement(
      const FlowBlock *bl); ///< Emit any required label statement for a given
                            ///< control-flow block
  void emitCommentGroup(
      const PcodeOp *inst); ///< Emit comments associated with a given statement
  void emitCommentBlockTree(
      const FlowBlock
          *bl); ///< Emit any comments under the given control-flow subtree
  ...
  void opFunc(const PcodeOp *op); ///< Push a \e functional expression based on
                                  ///< the given p-code op to the RPN stack
  void opTypeCast(const PcodeOp *op); ///< Push the given p-code op using
                                      ///< type-cast syntax to the RPN stack
  void opHiddenFunc(
      const PcodeOp *op); ///< Push the given p-code op as a hidden token
  static void
  printCharHexEscape(std::ostream &s,
                     int4 val); ///< Print value as an escaped hex sequence
  bool printCharacterConstant(std::ostream &s, const Address &addr,
                              Datatype *charType) const;
  int4 getHiddenThisSlot(
      const PcodeOp *op,
      FuncProto *fc); ///< Get position of "this" pointer needing to be hidden
  void resetDefaultsPrintC(
      void); ///< Set default values for options specific to PrintC
  ...
  virtual void pushSymbol(const Symbol *sym, const Varnode *vn,
                          const PcodeOp *op);
  virtual void pushUnnamedLocation(const Address &addr, const Varnode *vn,
                                   const PcodeOp *op);
...
public:
  BrlyC(Architecture *g,
        const std::string &nm = "brly-c-language"); ///< Constructor
...
/// \brief Set of print commands for displaying an open brace '{' and setting a
/// new indent level
///
/// These are the print commands sent to the emitter prior to printing and \e
/// else block. The open brace can be canceled if the block decides it wants to
/// use "else if" syntax.
class BrlyPendingBrace : public PendPrint {
  int4 indentId; ///< Id associated with the new indent level
public:
  BrlyPendingBrace(void) { indentId = -1; } ///< Constructor
...
/// \brief Push a token indicating a PTRSUB (a -> operator) is acting at an
/// offset from the original pointer
///
/// When a variable has TypePointerRel as its data-type, PTRSUB acts relative to
/// the \e parent data-type.  We print a specific token to indicate this
/// relative shift is happening. \param op is is the PTRSUB op
inline void BrlyC::pushTypePointerRel(const PcodeOp *op)
