...
using CastStrategyC = ghidra::CastStrategyC;
using PrintLanguage = ghidra::PrintLanguage;
using PrintLanguageCapability = ghidra::PrintLanguageCapability;
using OpCode = ghidra::OpCode;
using OpToken = ghidra::OpToken;
using TypeOp = ghidra::TypeOp;
using TypeOpSubpiece = ghidra::TypeOpSubpiece;
using UserPcodeOp = ghidra::UserPcodeOp;
using TypeField = ghidra::TypeField;
using Datatype = ghidra::Datatype;
using Emit = ghidra::Emit;
using EmitMarkup = ghidra::EmitMarkup;
using PendPrint = ghidra::PendPrint;
// using Atom = ghidra::Atom;
using TypeFactory = ghidra::TypeFactory;
using TypeCode = ghidra::TypeCode;
using TypeChar = ghidra::TypeChar;
using TypeEnum = ghidra::TypeEnum;
using TypeStruct = ghidra::TypeStruct;
using TypePointer = ghidra::TypePointer;
using TypePointerRel = ghidra::TypePointerRel;
using TypeUnion = ghidra::TypeUnion;
using TypeArray = ghidra::TypeArray;
using TypeBase = ghidra::TypeBase;
using TypeSpacebase = ghidra::TypeSpacebase;
using Comment = ghidra::Comment;
using CommentSorter = ghidra::CommentSorter;
using Symbol = ghidra::Symbol;
using Funcdata = ghidra::Funcdata;
using FuncProto = ghidra::FuncProto;
using FuncCallSpecs = ghidra::FuncCallSpecs;
using BlockBasic = ghidra::BlockBasic;
using BlockGraph = ghidra::BlockGraph;
using BlockCopy = ghidra::BlockCopy;
using BlockGoto = ghidra::BlockGoto;
using BlockList = ghidra::BlockList;
using BlockCondition = ghidra::BlockCondition;
using BlockIf = ghidra::BlockIf;
using BlockWhileDo = ghidra::BlockWhileDo;
using BlockDoWhile = ghidra::BlockDoWhile;
using BlockInfLoop = ghidra::BlockInfLoop;
using BlockSwitch = ghidra::BlockSwitch;
using Scope = ghidra::Scope;
using ScopeMap = ghidra::ScopeMap;
using HighVariable = ghidra::HighVariable;
using Varnode = ghidra::Varnode;
using FlowBlock = ghidra::FlowBlock;
using StringManager = ghidra::StringManager;
using ResolvedUnion = ghidra::ResolvedUnion;
// using ReversePolish = ghidra::ReversePolish;
...
using EquateSymbol = ghidra::EquateSymbol;
using SymbolEntry = ghidra::SymbolEntry;
using ProtoParameter = ghidra::ProtoParameter;
using FloatFormat = ghidra::FloatFormat;
using CPoolRecord = ghidra::CPoolRecord;

using LowlevelError = ghidra::LowlevelError;
using RecovError = ghidra::RecovError;

using int4 = ghidra::int4;
using int8 = ghidra::int8;
...
OpToken BrlyC::hidden = {
    "", "", 1, 70, false, OpToken::hiddenfunction, 0, 0, (OpToken *)0};
OpToken BrlyC::scope = {"::", "", 2,           70, true, OpToken::binary,
                        0,    0,  (OpToken *)0};
OpToken BrlyC::object_member = {".", "", 2,           66, true, OpToken::binary,
                                0,   0,  (OpToken *)0};
OpToken BrlyC::pointer_member = {
    "->", "", 2, 66, true, OpToken::binary, 0, 0, (OpToken *)0};
OpToken BrlyC::subscript = {
    "[", "]", 2, 66, false, OpToken::postsurround, 0, 0, (OpToken *)0};
OpToken BrlyC::function_call = {
    "(", ")", 2, 66, false, OpToken::postsurround, 0, 10, (OpToken *)0};
OpToken BrlyC::bitwise_not = {
    "~", "", 1, 62, false, OpToken::unary_prefix, 0, 0, (OpToken *)0};
OpToken BrlyC::boolean_not = {
    "!", "", 1, 62, false, OpToken::unary_prefix, 0, 0, (OpToken *)0};
OpToken BrlyC::unary_minus = {
    "-", "", 1, 62, false, OpToken::unary_prefix, 0, 0, (OpToken *)0};
OpToken BrlyC::unary_plus = {
    "+", "", 1, 62, false, OpToken::unary_prefix, 0, 0, (OpToken *)0};
OpToken BrlyC::addressof = {
    "&", "", 1, 62, false, OpToken::unary_prefix, 0, 0, (OpToken *)0};
OpToken BrlyC::dereference = {
    "*", "", 1, 62, false, OpToken::unary_prefix, 0, 0, (OpToken *)0};
OpToken BrlyC::typecast = {
    "(", ")", 2, 62, false, OpToken::presurround, 0, 0, (OpToken *)0};
OpToken BrlyC::multiply = {"*", "", 2,           54, true, OpToken::binary,
                           1,   0,  (OpToken *)0};
OpToken BrlyC::divide = {"/", "", 2,           54, false, OpToken::binary,
                         1,   0,  (OpToken *)0};
OpToken BrlyC::modulo = {"%", "", 2,           54, false, OpToken::binary,
                         1,   0,  (OpToken *)0};
OpToken BrlyC::binary_plus = {"+", "", 2,           50, true, OpToken::binary,
                              1,   0,  (OpToken *)0};
OpToken BrlyC::binary_minus = {"-", "", 2,           50, false, OpToken::binary,
                               1,   0,  (OpToken *)0};
OpToken BrlyC::shift_left = {"<<", "", 2,           46, false, OpToken::binary,
                             1,    0,  (OpToken *)0};
OpToken BrlyC::shift_right = {">>", "", 2,           46, false, OpToken::binary,
                              1,    0,  (OpToken *)0};
OpToken BrlyC::shift_sright = {
    ">>", "", 2, 46, false, OpToken::binary, 1, 0, (OpToken *)0};
OpToken BrlyC::less_than = {"<", "", 2,           42, false, OpToken::binary,
                            1,   0,  (OpToken *)0};
OpToken BrlyC::less_equal = {"<=", "", 2,           42, false, OpToken::binary,
                             1,    0,  (OpToken *)0};
OpToken BrlyC::greater_than = {">", "", 2,           42, false, OpToken::binary,
                               1,   0,  (OpToken *)0};
OpToken BrlyC::greater_equal = {
    ">=", "", 2, 42, false, OpToken::binary, 1, 0, (OpToken *)0};
OpToken BrlyC::equal = {"==", "", 2,           38, false, OpToken::binary,
                        1,    0,  (OpToken *)0};
OpToken BrlyC::not_equal = {"!=", "", 2,           38, false, OpToken::binary,
                            1,    0,  (OpToken *)0};
OpToken BrlyC::bitwise_and = {"&", "", 2,           34, true, OpToken::binary,
                              1,   0,  (OpToken *)0};
OpToken BrlyC::bitwise_xor = {"^", "", 2,           30, true, OpToken::binary,
                              1,   0,  (OpToken *)0};
OpToken BrlyC::bitwise_or = {"|", "", 2,           26, true, OpToken::binary,
                             1,   0,  (OpToken *)0};
OpToken BrlyC::boolean_and = {"&&", "", 2,           22, false, OpToken::binary,
                              1,    0,  (OpToken *)0};
OpToken BrlyC::boolean_xor = {"^^", "", 2,           20, false, OpToken::binary,
                              1,    0,  (OpToken *)0};
OpToken BrlyC::boolean_or = {"||", "", 2,           18, false, OpToken::binary,
                             1,    0,  (OpToken *)0};
OpToken BrlyC::assignment = {"=", "", 2,           14, false, OpToken::binary,
                             1,   5,  (OpToken *)0};
OpToken BrlyC::comma = {", ", "", 2,           2, true, OpToken::binary,
                        0,    0,  (OpToken *)0};
OpToken BrlyC::new_op = {"", "", 2,           62, false, OpToken::space,
                         1,  0,  (OpToken *)0};

// Inplace assignment operators
OpToken BrlyC::multequal = {"*=", "", 2,           14, false, OpToken::binary,
                            1,    5,  (OpToken *)0};
OpToken BrlyC::divequal = {"/=", "", 2,           14, false, OpToken::binary,
                           1,    5,  (OpToken *)0};
OpToken BrlyC::remequal = {"%=", "", 2,           14, false, OpToken::binary,
                           1,    5,  (OpToken *)0};
OpToken BrlyC::plusequal = {"+=", "", 2,           14, false, OpToken::binary,
                            1,    5,  (OpToken *)0};
OpToken BrlyC::minusequal = {"-=", "", 2,           14, false, OpToken::binary,
                             1,    5,  (OpToken *)0};
OpToken BrlyC::leftequal = {"<<=",           "", 2, 14,          false,
                            OpToken::binary, 1,  5, (OpToken *)0};
OpToken BrlyC::rightequal = {">>=",           "", 2, 14,          false,
                             OpToken::binary, 1,  5, (OpToken *)0};
OpToken BrlyC::andequal = {"&=", "", 2,           14, false, OpToken::binary,
                           1,    5,  (OpToken *)0};
OpToken BrlyC::orequal = {"|=", "", 2,           14, false, OpToken::binary,
                          1,    5,  (OpToken *)0};
OpToken BrlyC::xorequal = {"^=", "", 2,           14, false, OpToken::binary,
                           1,    5,  (OpToken *)0};

// Operator tokens for type expressions
OpToken BrlyC::type_expr_space = {
    "", "", 2, 10, false, OpToken::space, 1, 0, (OpToken *)0};
OpToken BrlyC::type_expr_nospace = {
    "", "", 2, 10, false, OpToken::space, 0, 0, (OpToken *)0};
OpToken BrlyC::ptr_expr = {
    "*", "", 1, 62, false, OpToken::unary_prefix, 0, 0, (OpToken *)0};
OpToken BrlyC::array_expr = {
    "[", "]", 2, 66, false, OpToken::postsurround, 1, 0, (OpToken *)0};
OpToken BrlyC::enum_cat = {"|", "", 2,           26, true, OpToken::binary,
                           0,   0,  (OpToken *)0};

const std::string BrlyC::EMPTY_STRING = "";
const std::string BrlyC::OPEN_CURLY = "{";
const std::string BrlyC::CLOSE_CURLY = "}";
const std::string BrlyC::SEMICOLON = ";";
const std::string BrlyC::COLON = ":";
const std::string BrlyC::EQUALSIGN = "=";
const std::string BrlyC::COMMA = ",";
const std::string BrlyC::DOTDOTDOT = "...";
const std::string BrlyC::KEYWORD_VOID = "void";
const std::string BrlyC::KEYWORD_TRUE = "true";
const std::string BrlyC::KEYWORD_FALSE = "false";
const std::string BrlyC::KEYWORD_IF = "if";
const std::string BrlyC::KEYWORD_ELSE = "else";
const std::string BrlyC::KEYWORD_DO = "do";
const std::string BrlyC::KEYWORD_WHILE = "while";
const std::string BrlyC::KEYWORD_FOR = "for";
const std::string BrlyC::KEYWORD_GOTO = "goto";
const std::string BrlyC::KEYWORD_BREAK = "break";
const std::string BrlyC::KEYWORD_CONTINUE = "continue";
const std::string BrlyC::KEYWORD_CASE = "case";
const std::string BrlyC::KEYWORD_SWITCH = "switch";
const std::string BrlyC::KEYWORD_DEFAULT = "default";
const std::string BrlyC::KEYWORD_RETURN = "return";
const std::string BrlyC::KEYWORD_NEW = "new";
const std::string BrlyC::typePointerRelToken = "ADJ";

// Constructing this registers the capability
BrlyCCapability BrlyCCapability::printCCapability;

BrlyCCapability::BrlyCCapability(void)

{
  name = "brly-c-language";
  isdefault = false;
}

PrintLanguage *BrlyCCapability::buildLanguage(Architecture *glb)

{
  return new BrlyC(glb, name);
}

/// \param g is the Architecture owning this c-language emitter
/// \param nm is the name assigned to this emitter
BrlyC::BrlyC(Architecture *g, const std::string &nm)
    : PrintLanguage(g, nm)
...
/// Push nested components of a data-type declaration onto a stack, so we can
/// access it bottom up \param ct is the data-type being emitted \param
/// typestack will hold the sub-types involved in the displaying the declaration
void BrlyC::buildTypeStack(const Datatype *ct,
...

{
  for (;;) {
    ...
    if (ct->getName().size() != 0) // This can be a base type
      break;
    if (ct->getMetatype() == ghidra::TYPE_PTR)
      ct = ((TypePointer *)ct)->getPtrTo();
    else if (ct->getMetatype() == ghidra::TYPE_ARRAY)
      ct = ((TypeArray *)ct)->getBase();
    else if (ct->getMetatype() == ghidra::TYPE_CODE) {
...
/// Push the comma separated list of data-type declarations onto the RPN stack
/// as part of emitting a given function prototype \param proto is the given
/// function prototype
void BrlyC::pushPrototypeInputs(const FuncProto *proto)

{
  ...
  if ((sz == 0) && (!proto->isDotdotdot()))
    ...
    for (int4 i = 0; i < sz - 1; ++i)
      pushOp(&comma,
             (const PcodeOp *)0); // Print a comma for each parameter (above 1)
    if (proto->isDotdotdot() &&
        (sz != 0)) // Print comma for dotdotdot (if it is not by itself)
    ...
    if (proto->isDotdotdot()) {
      if (sz != 0)
        ...
        // In ANSI C, a prototype with empty parens means the parameters are
        // unspecified (not void) In C++, empty parens mean void, we use the
        // ANSI C convention
        pushAtom(Atom(EMPTY_STRING, blanktoken,
                      EmitMarkup::no_color)); // An empty list of parameters
      }
    }
  }
}

/// Calculate what elements of a given symbol's namespace path are necessary to
/// distinguish it within the current scope. Then print these elements. \param
/// symbol is the given symbol
void BrlyC::pushSymbolScope(const Symbol *symbol)

{
  ...
  if (scopedepth != 0) {
    ...
    for (int4 i = scopedepth - 1; i >= 0; --i) {
      pushAtom(Atom(scopeList[i]->getName(), syntax, EmitMarkup::global_color,
                    (PcodeOp *)0, (Varnode *)0));
    }
  }
}

/// Emit the elements of the given symbol's namespace path that distinguish it
/// within the current scope. \param symbol is the given Symbol
void BrlyC::emitSymbolScope(const Symbol *symbol)
...
/// \param noident is \b true if an identifier will not be pushed as part of the
/// declaration
void BrlyC::pushTypeStart(const Datatype *ct, bool noident)

{
  ...
  if (ct->getName().size() == 0) { // Check for anonymous type
    ...
    std::string nm = genericTypeName(ct);
...
void BrlyC::pushTypeEnd(const Datatype *ct)

{
  ...
  for (;;) {
    ...
    if (ct->getMetatype() == ghidra::TYPE_PTR)
      ...
      ct = ctarray->getBase();
      push_integer(ctarray->numElements(), 4, false, syntax, (const Varnode *)0,
                   (const PcodeOp *)0);
    } else if (ct->getMetatype() == ghidra::TYPE_CODE) {
...
/// \return \b false if '*' syntax is required, \b true if some other syntax is
/// used
bool BrlyC::checkArrayDeref(const Varnode *vn) const

{
  ...
  if (!vn->isWritten())
    return false;
  op = vn->getDef();
  if (op->code() == ghidra::CPUI_SEGMENTOP) {
    vn = op->getIn(2);
    if (!vn->isImplied())
      return false;
    if (!vn->isWritten())
  ...
  if ((op->code() != ghidra::CPUI_PTRSUB) &&
      (op->code() != ghidra::CPUI_PTRADD))
    return false;
  return true;
}

/// This is used for expression that require functional syntax, where the name
/// of the function is the name of the operator. The inputs to the p-code op
/// form the roots of the comma separated list of \e parameters within the
/// syntax. \param op is the given PcodeOp
void BrlyC::opFunc(const PcodeOp *op)
...
/// The syntax represents the given op using a standard c-language cast.  The
/// data-type being cast to is obtained from the output variable of the op. The
/// input expression is also recursively pushed. \param op is the given PcodeOp
void BrlyC::opTypeCast(const PcodeOp *op)
...
/// where the function name is not printed. The input expression is simply
/// printed without adornment inside the larger expression, with one minor
/// difference. The hidden operator protects against confusing evaluation order
/// between the operators inside and outside the hidden function.  If both the
/// inside and outside operators are the same associative token, the hidden
/// token makes sure the inner expression is surrounded with parentheses. \param
/// op is the given PcodeOp
void BrlyC::opHiddenFunc(const PcodeOp *op)
...
void BrlyC::opCopy(const PcodeOp *op)

{
  pushVn(op->getIn(0), op, mods);
}

void BrlyC::opLoad(const PcodeOp *op)
...
void BrlyC::opStore(const PcodeOp *op)
...
void BrlyC::opBranch(const PcodeOp *op)
...
void BrlyC::opCbranch(const PcodeOp *op)
...
void BrlyC::opBranchind(const PcodeOp *op)

{
  // FIXME:  This routine shouldn't emit directly
  emit->tagOp(KEYWORD_SWITCH, EmitMarkup::keyword_color,
              op); // Print header for switch
...
void BrlyC::opCall(const PcodeOp *op)

{
  pushOp(&function_call, op);

  int4 id = emit->beginFunctionCall(op);

  const Varnode *callpoint = op->getIn(0);
  FuncCallSpecs *fc;
  if (callpoint->getSpace()->getType() == ghidra::IPTR_FSPEC) {
    fc = FuncCallSpecs::getFspecFromConst(callpoint->getAddr());
    if (fc->getName().size() == 0) {
      std::string nm = genericFunctionName(fc->getEntryAddress());
      pushAtom(Atom(nm, functoken, EmitMarkup::funcname_color, op,
                    (const Funcdata *)0));
    } else {
      Funcdata *fd = fc->getFuncdata();
      if (fd != (Funcdata *)0)
        pushSymbolScope(fd->getSymbol());
      pushAtom(Atom(fc->getName(), functoken, EmitMarkup::funcname_color, op,
                    (const Funcdata *)0));
    }
  } else {
    clear();
  ...
  recurse();
  emit->endFunctionCall(id);
}

void BrlyC::opCallind(const PcodeOp *op)
...
void BrlyC::opCallother(const PcodeOp *op)

{
  ...
  if (display == UserPcodeOp::annotation_assignment) {
    ...
    if (op->numInput() > 1) {
      ...
      pushAtom(Atom(EMPTY_STRING, blanktoken,
                    EmitMarkup::no_color)); // Push empty token for void
  }
}

void BrlyC::opConstructor(const PcodeOp *op, bool withNew)

{
  ...
  if (withNew) {
    ...
    pushOp(&new_op, newop);
    pushAtom(
        Atom(KEYWORD_NEW, optoken, EmitMarkup::keyword_color, newop, outvn));
    dt = outvn->getTypeDefFacing();
  } else {
    const Varnode *thisvn = op->getIn(1);
    dt = thisvn->getType();
  }
  if (dt->getMetatype() == ghidra::TYPE_PTR) {
    dt = ((TypePointer *)dt)->getPtrTo();
  }
  std::string nm = dt->getName();
  pushOp(&function_call, op);
  pushAtom(Atom(nm, optoken, EmitMarkup::funcname_color, op));
  // implied vn's pushed on in reverse order for efficiency
  // see PrintLanguage::pushVnImplied
...
void BrlyC::opReturn(const PcodeOp *op)
...
void BrlyC::opIntZext(const PcodeOp *op, const PcodeOp *readOp)
...
void BrlyC::opIntSext(const PcodeOp *op, const PcodeOp *readOp)
...
/// Print the BOOL_NEGATE but check for opportunities to flip the next operator
/// instead \param op is the BOOL_NEGATE PcodeOp
void BrlyC::opBoolNegate(const PcodeOp *op)

{
  if (isSet(negatetoken)) { // Check if we are negated by a previous BOOL_NEGATE
    ...
    pushVn(op->getIn(0), op,
           mods); // Don't print ourselves, but print our input unmodified
  } else if (checkPrintNegation(
                 op->getIn(0))) { // If the next operator can be flipped
    pushVn(
        op->getIn(0), op,
        mods |
            negatetoken); // Don't print ourselves, but print a modified input
  } else {
...
void BrlyC::opSubpiece(const PcodeOp *op)

{
  if (op->doesSpecialPrinting()) { // Special printing means it is a field
                                   // extraction
    ...
    if (ct->isPieceStructured()) {
      ...
      const TypeField *field =
          ct->findTruncation(byteOff, op->getOut()->getSize(), op, 1,
                             offset); // Use artificial slot
      if (field != (const TypeField *)0 &&
          offset == 0) { // A formal structure field
        pushOp(&object_member, op);
        pushVn(vn, op, mods);
        pushAtom(Atom(field->name, fieldtoken, EmitMarkup::no_color, ct,
                      field->ident, op));
        return;
      } else if (vn->isExplicit() &&
                 vn->getHigh()->getSymbolOffset() ==
                     -1) { // An explicit, entire, structured object
        Symbol *sym = vn->getHigh()->getSymbol();
        if (sym != (Symbol *)0) {
          int4 sz = op->getOut()->getSize();
          int4 off = (int4)op->getIn(1)->getOffset();
          off =
              vn->getSpace()->isBigEndian() ? vn->getSize() - (sz + off) : off;
...
void BrlyC::opPtradd(const PcodeOp *op)

{
  ...
  if (!printval) {
    ...
    if (tp->getMetatype() == ghidra::TYPE_PTR) {
      if (tp->getPtrTo()->getMetatype() == ghidra::TYPE_ARRAY)
        printval = true;
    }
  }
...

{
  if ((vn->isImplied()) && (vn->isWritten())) {
    ...
    if (def->code() == ghidra::CPUI_PTRSUB)
      return true;
    if (def->code() == ghidra::CPUI_PTRADD)
      return true;
  }
  return false;
}
...
void BrlyC::opPtrsub(const PcodeOp *op)

{
  ...
  if (ptype->getMetatype() != ghidra::TYPE_PTR) {
    clear();
  ...
  if (ct->getMetatype() == ghidra::TYPE_STRUCT ||
      ct->getMetatype() == ghidra::TYPE_UNION) {
    uintb suboff = in1const; // How far into container
    if (ptrel != (TypePointerRel *)0) {
      suboff += ptrel->getPointerOffset();
      suboff &= ghidra::calc_mask(ptype->getSize());
      if (suboff == 0) {
    ...
    int4 fieldid;
    int8 newoff;
    if (ct->getMetatype() == ghidra::TYPE_UNION) {
      if (suboff != 0)
      ...
      if (resUnion == (const ResolvedUnion *)0 || resUnion->getFieldNum() < 0)
        throw LowlevelError(
            "PTRSUB for union that does not resolve to a field");
      const TypeField *fld =
          ((TypeUnion *)ct)->getField(resUnion->getFieldNum());
      fieldid = fld->ident;
      fieldname = fld->name;
      fieldtype = fld->type;
    } else { // ghidra::TYPE_STRUCT
    ...
    arrayvalue = false;
    // The '&' is dropped if the output type is an array
    if ((fieldtype != (Datatype *)0) &&
        (fieldtype->getMetatype() == ghidra::TYPE_ARRAY)) {
    ...
    if (!valueon) { // Printing an ampersand
      if (flex) {   // EMIT  &( ).name
        ...
        pushVn(in0, op, m | print_load_value);
        pushAtom(
            Atom(fieldname, fieldtoken, EmitMarkup::no_color, ct, fieldid, op));
      } else { // EMIT  &( )->name
        pushOp(&addressof, op);
  ...
  } else if (ct->getMetatype() == ghidra::TYPE_SPACEBASE) {
    ...
    if (symbol != (Symbol *)0) {
      ct = symbol->getType();
      // The '&' is dropped if the output type is an array
      if (ct->getMetatype() == ghidra::TYPE_ARRAY) {
        arrayvalue = valueon; // If printing value, use [0]
    ...
    if (symbol == (Symbol *)0) {
      ...
      if (off == 0)
        ...
        //	Datatype *exttype = ((mods & print_store_value)!=0) ? (Datatype
        //*)0 : ct;
...
/// - slot 1 is the segment, we could conceivably try to annotate the segment
/// here
/// - slot 2 is the pointer we are really interested in printing
///
/// \param op is the SEGMENTOP PcodeOp
void BrlyC::opSegmentOp(const PcodeOp *op)

{
  pushVn(op->getIn(2), op, mods);
}

void BrlyC::opCpoolRefOp(const PcodeOp *op)

{
  ...
  if (rec == (const CPoolRecord *)0) {
    ...
    case CPoolRecord::class_reference:
      pushAtom(
          Atom(rec->getToken(), vartoken, EmitMarkup::type_color, op, outvn));
      break;
    case CPoolRecord::instance_of: {
      Datatype *dt = rec->getType();
      while (dt->getMetatype() == ghidra::TYPE_PTR) {
        dt = ((TypePointer *)dt)->getPtrTo();
      }
      pushOp(&function_call, op);
      pushAtom(Atom(rec->getToken(), functoken, EmitMarkup::funcname_color, op,
                    outvn));
      pushOp(&comma, (const PcodeOp *)0);
      pushVn(vn0, op, mods);
    ...
    default: {
      ...
      if (ct->getMetatype() == ghidra::TYPE_PTR) {
        ct = ((TypePointer *)ct)->getPtrTo();
        if (ct->getMetatype() == ghidra::TYPE_CODE)
...
void BrlyC::opNewOp(const PcodeOp *op)

{
  ...
  if (op->numInput() == 2) {
    ...
    if (!vn0->isConstant()) {
      ...
      pushOp(&new_op, op);
      pushAtom(
          Atom(KEYWORD_NEW, optoken, EmitMarkup::keyword_color, op, outvn));
      std::string nm;
      if (outvn == (const Varnode *)0) { // Its technically possible, for new
                                         // result to be unused
        nm = "<unused>";
      } else {
  ...
  // This printing is used only if the 'new' operator doesn't feed directly into
  // a constructor
  pushOp(&function_call, op);
...
void BrlyC::opInsertOp(const PcodeOp *op)

{
  opFunc(op); // If no other way to print it, print as functional operator
}

void BrlyC::opExtractOp(const PcodeOp *op)
...
///
/// Various checks are made to see if the integer should be printed as an \e
/// equate symbol or if there is other overriding information about what format
/// it should be printed in. In any case, a final determination of the format is
/// made and the integer is pushed as a single token. \param val is the given
/// integer value \param sz is the size (in bytes) to associate with the integer
/// \param sign is set to \b true if the integer should be treated as a signed
/// value \param vn is the Varnode holding the value \param op is the PcodeOp
/// using the value
void BrlyC::push_integer(uintb val, int4 sz, bool sign, tagtype tag,
  ...
  if ((vn != (const Varnode *)0) && (!vn->isAnnotation())) {
    ...
    if (displayFormat ==
        0) // The symbol's formatting overrides any formatting on the data-type
      displayFormat = high->getType()->getDisplayFormat();
  }
  if (sign &&
      displayFormat != Symbol::force_char) { // Print the constant as signed
    uintb mask = ghidra::calc_mask(sz);
    uintb flip = val ^ mask;
...
void BrlyC::push_float(uintb val, int4 sz, tagtype tag, const Varnode *vn,
                       const PcodeOp *op) {
  std::string token;
  ...
  if (format == (const FloatFormat *)0) {
    ...
    if (type == FloatFormat::infinity) {
      ...
      if ((mods & force_scinote) != 0) {
        ...
        t.unsetf(std::ios::floatfield); // Use "default" notation
...
void BrlyC::printUnicode(std::ostream &s, int4 onechar) const
...
void BrlyC::pushType(const Datatype *ct)
...
/// \param vn is the Varnode holding the value
/// \param op is the PcodeOp using the value
void BrlyC::pushBoolConstant(uintb val, const TypeBase *ct, tagtype tag,
                             const Varnode *vn, const PcodeOp *op) {
  if (val != 0)
...
/// \brief Return \b true if this language requires a prefix when expressing \e
/// wide characters
///
/// The c-language standard requires that strings (and character constants) made
/// up of \e wide character elements have an 'L' prefix added before the quote
/// characters.  Other related languages may not do this.  Having this as a
/// virtual method lets derived languages to tailor their strings while still
/// using the basic PrintC functionality \return \b true if a prefix should be
/// printed
bool BrlyC::doEmitWideCharPrefix(void) const { return true; }

/// Print the given value using the standard character hexadecimal escape
/// sequence. \param s is the stream to write to \param val is the given value
void BrlyC::printCharHexEscape(std::ostream &s, int4 val)

{
  if (val < 256) {
    s << "\\x" << std::setfill('0') << std::setw(2) << std::hex << val;
  } else if (val < 65536) {
    s << "\\x" << std::setfill('0') << std::setw(4) << std::hex << val;
  } else
    s << "\\x" << std::setfill('0') << std::setw(8) << std::hex << val;
}

/// \brief Print a quoted (unicode) std::string at the given address.
///
/// Data for the std::string is obtained directly from the LoadImage.  The bytes
/// are checked for appropriate unicode encoding and the presence of a
/// terminator. If all these checks pass, the std::string is emitted. \param s
/// is the output stream to print to \param addr is the address of the
/// std::string data within the LoadImage \param charType is the underlying
/// character data-type \return \b true if a proper std::string was found and
/// printed to the stream
bool BrlyC::printCharacterConstant(std::ostream &s, const Address &addr,
                                   Datatype *charType) const

{
  ...
  bool isTrunc = false;
  const std::vector<uint1> &buffer(
      manager->getStringData(addr, charType, isTrunc));
  if (buffer.empty())
    return false;
  if (doEmitWideCharPrefix() && charType->getSize() > 1 &&
      !charType->isOpaqueString())
...
/// For the given CALL op, if a "this" pointer exists and needs to be hidden
/// because of the print configuration, return the Varnode slot corresponding to
/// the "this". Otherwise return -1. \param op is the given CALL PcodeOp \param
/// fc is the function prototype corresponding to the CALL \return the "this"
/// Varnode slot or -1
int4 BrlyC::getHiddenThisSlot(const PcodeOp *op, FuncProto *fc)
...
void BrlyC::resetDefaultsPrintC(void)

{
  option_convention = false;
  option_hide_exts = true;
  option_inplace_ops = true;
  option_nocasts = true;
  option_NULL = true;
...
///
/// For C, a character constant is usually emitted as the character in single
/// quotes. Handle unicode, wide characters, etc. Characters come in with the
/// compiler's raw encoding. \param val is the constant value \param ct is
/// data-type attached to the value \param vn is the Varnode holding the value
/// \param op is the PcodeOp using the value
void BrlyC::pushCharConstant(uintb val, const Datatype *ct, tagtype tag,
                             const Varnode *vn, const PcodeOp *op)

{
  uint4 displayFormat = 0;
  bool isSigned = (ct->getMetatype() == ghidra::TYPE_INT);
  if ((vn != (const Varnode *)0) && (!vn->isAnnotation())) {
    HighVariable *high = vn->getHigh();
    Symbol *sym = high->getSymbol();
    if (sym != (Symbol *)0) {
  ...
  if ((ct->getSize() == 1) && (val >= 0x80)) {
    // For byte characters, the encoding is assumed to be ASCII, UTF-8, or some
    // other code-page that extends ASCII. At 0x80 and above, we cannot treat
    // the value as a unicode code-point. Its either part of a multi-byte UTF-8
    // encoding or an unknown code-page value. In either case, we print as an
    // integer or an escape sequence.
    if (displayFormat != Symbol::force_hex &&
        displayFormat != Symbol::force_char) {
      push_integer(val, 1, isSigned, tag, vn, op);
      return;
    }
    displayFormat =
        Symbol::force_hex; // Fallthru but force a hex representation
  }
  std::ostringstream t;
  // From here we assume, the constant value is a direct unicode code-point.
  // The value could be an illegal code-point (surrogates or beyond the max
  // code-point), but this will just be emitted as an escape sequence.
...
/// \param vn is the Varnode holding the value
/// \param op is the PcodeOp using the value
void BrlyC::pushEnumConstant(uintb val, const TypeEnum *ct, tagtype tag,
                             const Varnode *vn, const PcodeOp *op) {
  ...
  if (valnames.size() > 0) {
    ...
    //    std::ostringstream s;
    //    s << "BAD_ENUM(0x" << std::hex << val << ")";
    //    pushAtom(Atom(s.str(),vartoken,EmitMarkup::const_color,op,vn));
  }
}

/// \brief Attempt to push a quoted std::string representing a given constant
/// pointer onto the RPN stack
///
/// Check if the constant pointer refers to character data that can be emitted
/// as a quoted string. If so push the string, if not return \b false to
/// indicate a token was not pushed \param val is the value of the given
/// constant pointer \param ct is the pointer data-type attached to the value
...
bool BrlyC::pushPtrCharConstant(uintb val, const TypePointer *ct,
                                const Varnode *vn, const PcodeOp *op)

{
  ...
  if (op != (const PcodeOp *)0)
    point = op->getAddr();
  Address stringaddr =
      glb->resolveConstant(spc, val, ct->getSize(), point, fullEncoding);
...
/// \brief Attempt to push a function name representing a constant pointer onto
/// the RPN stack
///
/// Given the pointer value, try to look up the function at that address and
/// push the function's name as a single Atom. \param val is the given constant
/// pointer value \param ct is the pointer data-type attached to the value
/// \param vn is the Varnode holding the value
/// \param op is the PcodeOp using the value
/// \return \b true if a name was pushed to the RPN stack, return \b false
/// otherwise
bool BrlyC::pushPtrCodeConstant(uintb val, const TypePointer *ct,
                                const Varnode *vn, const PcodeOp *op) {
  ...
  if (fd != (Funcdata *)0) {
    pushAtom(
        Atom(fd->getName(), functoken, EmitMarkup::funcname_color, op, fd));
    return true;
  }
  return false;
}

void BrlyC::pushConstant(uintb val, const Datatype *ct, tagtype tag,
                         const Varnode *vn, const PcodeOp *op) {
  Datatype *subtype;
  ...
  case ghidra::TYPE_PTRREL:
    ...
    if (subtype->isCharPrint()) {
      if (pushPtrCharConstant(val, (const TypePointer *)ct, vn, op))
        return;
    } else if (subtype->getMetatype() == ghidra::TYPE_CODE) {
...
bool BrlyC::pushEquate(uintb val, int4 sz, const EquateSymbol *sym,
                       const Varnode *vn, const PcodeOp *op)

{
  ...
  if (modval != baseval) { // If 1-bits are getting masked
    if (ghidra::sign_extend(modval, sz, sizeof(uintb)) !=
...
void BrlyC::pushAnnotation(const Varnode *vn, const PcodeOp *op)

{
  ...
  if (entry != (SymbolEntry *)0) {
    ...
    std::string regname =
        glb->translate->getRegisterName(vn->getSpace(), vn->getOffset(), size);
    if (regname.empty()) {
      AddrSpace *spc = vn->getSpace();
      std::string spacename = spc->getName();
      spacename[0] = toupper(spacename[0]); // Capitalize space
      std::ostringstream s;
      s << spacename;
      s << std::hex << std::setfill('0') << std::setw(2 * spc->getAddrSize());
...
void BrlyC::pushSymbol(const Symbol *sym, const Varnode *vn, const PcodeOp *op)

{
  ...
  if (sym->hasMergeProblems() && vn != (Varnode *)0) {
    ...
    if (high->isUnmerged()) {
      ...
      if (entry != (SymbolEntry *)0) {
        s << '$' << std::dec << entry->getSymbol()->getMapEntryPosition(entry);
      } else
        s << "$$";
...
void BrlyC::pushUnnamedLocation(const Address &addr, const Varnode *vn,
                                const PcodeOp *op) {
  std::ostringstream s;
...
void BrlyC::pushPartialSymbol(const Symbol *sym, int4 off, int4 sz,
                              const Varnode *vn, const PcodeOp *op,
                              int4 inslot) {
  ...
  std::vector<BrlyPartialSymbolEntry> stack;
  ...
  while (ct != (Datatype *)0) {
    if (off == 0) {
      if (sz == 0 ||
          (sz == ct->getSize() &&
           (!ct->needsResolution() || ct->getMetatype() == ghidra::TYPE_PTR)))
        break;
    }
    ...
    if (ct->getMetatype() == ghidra::TYPE_STRUCT) {
      ...
      if (field != (const TypeField *)0) {
        off = newoff;
        stack.emplace_back();
        BrlyPartialSymbolEntry &entry(stack.back());
    ...
    } else if (ct->getMetatype() == ghidra::TYPE_UNION) {
      const TypeField *field;
      field = ct->findTruncation(off, sz, op, inslot, newoff);
      if (field != (const TypeField *)0) {
        off = newoff;
      ...
      if (castStrategy->isSubpieceCastEndian(
              outtype, ct, off,
    ...
    if (!succeeded) { // Subtype was not good
      ...
      if (sz == 0)
        sz = ct->getSize() - off;
      entry.fieldname = unnamedField(
          off, sz); // If nothing else works, generate artificial field name
      entry.field = (const TypeField *)0;
      entry.hilite = EmitMarkup::no_color;
      ct = (Datatype *)0;
    }
  ...
  for (int4 i = 0; i < stack.size(); ++i) {
    ...
    if (field == (const TypeField *)0)
      ...
      pushAtom(Atom(stack[i].fieldname, fieldtoken, stack[i].hilite,
                    stack[i].parent, field->ident, op));
  }
}

void BrlyC::pushMismatchSymbol(const Symbol *sym, int4 off, int4 sz,
                               const Varnode *vn, const PcodeOp *op) {
  if (off == 0) {
...

{
  ...
  pushOp(&object_member, op);
  defOp->getOpcode()->push(this, defOp, op);
  pushAtom(Atom(field->name, fieldtoken, EmitMarkup::no_color, parent,
                field->ident, op));
}

/// Print all the components making up the data-type, using the \b struct
/// keyword \param ct is the structure data-type
void BrlyC::emitStructDefinition(const TypeStruct *ct)
...
/// Print all the named values making up the data-type, using the \b enum
/// keyword \param ct is the enumerated data-type
void BrlyC::emitEnumDefinition(const TypeEnum *ct)

{
  ...
  pushMod();
  bool sign = (ct->getMetatype() == ghidra::TYPE_INT);
  emit->tagLine();
  ...
  while (iter != ct->endEnum()) {
    ...
    emit->spaces(1);
    push_integer((*iter).first, ct->getSize(), sign, syntax, (Varnode *)0,
                 (PcodeOp *)0);
    recurse();
    emit->print(SEMICOLON);
    ++iter;
...
/// In C, when printing a function prototype, the function's output data-type is
/// displayed first as a type declaration, where the function name acts as the
/// declaration's identifier. This method emits the declaration in preparation
/// for this. \param proto is the function prototype object \param fd is the
/// (optional) Funcdata object providing additional meta-data about the function
void BrlyC::emitPrototypeOutput(const FuncProto *proto, const Funcdata *fd) {
  PcodeOp *op;
  Varnode *vn;
  ...
  if ((outtype->getMetatype() != ghidra::TYPE_VOID) && (op != (PcodeOp *)0))
    vn = op->getIn(1);
  else
    vn = (Varnode *)0;
...
/// This emits the individual type declarations of the input parameters to the
/// function as a comma separated list. \param proto is the given prototype of
/// the function
void BrlyC::emitPrototypeInputs(const FuncProto *proto)
...
void BrlyC::emitLocalVarDecls(const Funcdata *fd)
...
/// This emits an entire statement rooted at a given operation. All associated
/// expressions on the right-hand and left-hand sides are recursively emitted.
/// Depending on the current printing properties, the statement is usually
/// terminated with ';' character. \param inst is the given root PcodeOp of the
/// statement
void BrlyC::emitStatement(const PcodeOp *inst)
...
/// construct a statement with the appropriate c-language keyword (\b goto, \b
/// break, \b continue) representing a control-flow branch between the blocks.
...
void BrlyC::emitGotoStatement(const FlowBlock *bl, const FlowBlock *exp_bl,
                              uint4 type)
...
void BrlyC::initializeFromArchitecture(void)

{
  castStrategy->setTypeFactory(glb->types);
  if (glb->types->getSizeOfLong() ==
      glb->types->getSizeOfInt()) // If long and int sizes are the same
    sizeSuffix = "LL"; // Use "long long" suffix to indicate large integer
  else
    sizeSuffix = "L"; // Otherwise just use long suffix
}

void BrlyC::adjustTypeOperators(void)
...
void BrlyC::setCommentStyle(const std::string &nm)
...
void BrlyC::emitTypeDefinition(const Datatype *ct)
...
bool BrlyC::checkPrintNegation(const Varnode *vn)

{
  ...
  bool reorder = false;
  OpCode opc = get_booleanflip(
      op->code(),
      reorder); // This is the set of ops that can be negated as a token
  if (opc == ghidra::CPUI_MAX)
    return false;
  return true;
}

void BrlyC::docTypeDefinitions(const TypeFactory *typegrp)
...
/// Check that the given p-code op has an \e in-place token form and if the
/// first input and the output are references to  the same variable. If so, emit
/// the expression using the \e in-place token. \param op is the given PcodeOp
/// \return \b true if the expression was emitted (as in-place), or \b false if
/// not emitted at all
bool BrlyC::emitInplaceOp(const PcodeOp *op)
...
void BrlyC::emitExpression(const PcodeOp *op)
...
void BrlyC::emitVarDecl(const Symbol *sym)
...
void BrlyC::emitVarDeclStatement(const Symbol *sym)
...
bool BrlyC::emitScopeVarDecls(const Scope *symScope, int4 cat)

{
  ...
  if (cat >= 0) { // If a category is specified
    ...
    for (int4 i = 0; i < sz; ++i) {
      ...
      // Slightly different handling for categorized symbols (cat=1 is dynamic
      // symbols)
  ...
  for (; iter != enditer; ++iter) {
    ...
    if (sym->isMultiEntry()) {
      if (sym->getFirstWholeMap() != entry)
        continue; // Only emit the first SymbolEntry for declaration of
                  // multi-entry Symbol
    }
    notempty = true;
    emitVarDeclStatement(sym);
  }
  std::list<SymbolEntry>::const_iterator iter_d = symScope->beginDynamic();
  std::list<SymbolEntry>::const_iterator enditer_d = symScope->endDynamic();
...
void BrlyC::emitFunctionDeclaration(const Funcdata *fd) {
  const FuncProto *proto = &fd->getFuncProto();
  int4 id = emit->beginFuncProto();
  emitPrototypeOutput(proto, fd);

  if (proto->getOutputType()->getMetatype() != ghidra::TYPE_PTR) {
    emit->spaces(1);
  } else {
    auto tname = proto->getOutputType()->getName();
    if (!tname.empty() && *tname.rbegin() != '*') {
      emit->spaces(1);
    }
  }
  ...
  emit->tagFuncName(fd->getName(), EmitMarkup::funcname_color, fd,
                    (PcodeOp *)0);
...
void BrlyC::emitGlobalVarDeclsRecursive(Scope *symScope)
...
void BrlyC::docSingleGlobal(const Symbol *sym)
...
void BrlyC::docFunction(const Funcdata *fd)
...
void BrlyC::emitBlockBasic(const BlockBasic *bb)
  ...
  if (isSet(only_branch)) {
    ...
    for (iter = bb->beginOp(); iter != bb->endOp(); ++iter) {
      ...
      if (inst->isBranch()) {
        ...
        // the block classes
        if (inst->code() == ghidra::CPUI_BRANCH)
          continue;
      }
...
void BrlyC::emitBlockGraph(const BlockGraph *bl)
...
void BrlyC::emitBlockCopy(const BlockCopy *bl)
...
void BrlyC::emitBlockGoto(const BlockGoto *bl)
...
void BrlyC::emitBlockLs(const BlockList *bl)
...
void BrlyC::emitBlockCondition(const BlockCondition *bl)

{
  ...
  if (isSet(only_branch) || isSet(comma_separate)) {
    ...
    pol.op = (PcodeOp *)0;
    pol.visited = 1;
    if (bl->getOpcode() == ghidra::CPUI_BOOL_AND)
      pol.tok = &boolean_and;
    else
      pol.tok = &boolean_or;
    emitOp(pol);
...
void BrlyPendingBrace::callback(Emit *emit)

{
  emit->print(BrlyC::OPEN_CURLY);
  indentId = emit->startIndent();
}

void BrlyC::emitBlockIf(const BlockIf *bl)

{
  const PcodeOp *op;
  BrlyPendingBrace pendingBrace;
  ...
  if (emit->hasPendingPrint(
          &pendingBrace))       // If we issued a brace but it did not emit
    emit->cancelPendingPrint(); // Cancel the brace in order to have "else if"
                                // syntax
  else
...
void BrlyC::emitForLoop(const BlockWhileDo *bl)
...
void BrlyC::emitBlockWhileDo(const BlockWhileDo *bl)
...
void BrlyC::emitBlockDoWhile(const BlockDoWhile *bl)
...
void BrlyC::emitBlockInfLoop(const BlockInfLoop *bl)
...
void BrlyC::emitSwitchCase(int4 casenum, const BlockSwitch *switchbl)

{
  ...
  if (switchbl->isDefaultCase(casenum)) {
    ...
    for (i = 0; i < num; ++i) {
      ...
      emit->spaces(1);
      pushConstant(val, ct, casetoken, (Varnode *)0, (PcodeOp *)0);
      recurse();
      emit->print(COLON);
    }
  }
...
void BrlyC::emitLabel(const FlowBlock *bl)
  ...
  if (!bb->hasSpecialLabel()) {
    if (bb->getType() == FlowBlock::t_basic) {
      const Scope *symScope =
          ((const BlockBasic *)bb)->getFuncdata()->getScopeLocal();
...
void BrlyC::emitLabelStatement(const FlowBlock *bl)
...
/// The block does not have to be a basic block.  This routine finds the entry
/// basic block and prints any necessary labels for that. \param bl is the given
/// control-flow block
void BrlyC::emitAnyLabelStatement(const FlowBlock *bl)
...
void BrlyC::emitCommentGroup(const PcodeOp *inst)
...
/// With the control-flow hierarchy, print any comments associated with basic
/// blocks in the specified subtree.  Used where statements from multiple basic
/// blocks are printed on one line and a normal comment would get printed in the
/// middle of this line. \param bl is the root of the control-flow subtree
void BrlyC::emitCommentBlockTree(const FlowBlock *bl)
...
void BrlyC::emitCommentFuncHeader(const Funcdata *fd)

{
  ...
  if (option_unplaced) {
    ...
    while (commsorter.hasNext()) {
      ...
      if (!extralinebreak) {
        Comment label(
            Comment::warningheader, fd->getAddress(), fd->getAddress(), 0,
...
void BrlyC::emitBlockSwitch(const BlockSwitch *bl)

{
  ...
  for (int4 i = 0; i < bl->getNumCaseBlocks(); ++i) {
    ...
    if (bl->getGotoType(i) != 0) {
      emit->tagLine();
      emitGotoStatement(bl->getBlock(0), bl->getCaseBlock(i),
                        bl->getGotoType(i));
    } else {
      ...
      bl2->emit(this);
      if (bl->isExit(i) &&
          (i != bl->getNumCaseBlocks() -
                    1)) { // Blocks that formally exit the switch
        emit->tagLine();
        emitGotoStatement(
            bl2, (const FlowBlock *)0,
...
std::string BrlyC::genericFunctionName(const Address &addr)
...
/// \param ct is the given data-type
/// \return the generated name
std::string BrlyC::genericTypeName(const Datatype *ct)
