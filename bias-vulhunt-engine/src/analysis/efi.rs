...
use bias::platform::efi::{EFIComponentName, EFIModule, EFIStandalone, EFIStandaloneName};
...
pub struct VulHuntEFIAnalyser {
    checkers: CheckerDB,
    analyses: EFIModuleAnalysis,
    config: VulHuntEFIAnalyserConfig,
    symbols: FLIRTSymbolManager,
    types: TypeManager,
}

#[derive(Debug, Error)]
pub enum VulhuntEFIAnalyserError {
    #[error("cannot initialise type manager: {0}")]
    Types(#[from] TypeManagerError),
}

#[derive(Debug, Clone, Default)]
pub struct VulHuntEFIAnalyserConfig {
    pub render: bool,
    pub ignore_errors: bool,
    pub module_directory: VulHuntModuleDir,
}

impl VulHuntEFIAnalyserConfig {
    pub fn new() -> Self {
        Self::default()
    }
...
impl AnalysisGroupAnalyserForCode for VulHuntEFIAnalyser {
    fn decompiler_configuration(&self) -> Cow<'_, DecompilerConfig> {
        Cow::Borrowed(self.analyses.decompiler_configuration())
    }
    ...
    fn analyse_and_check(
        ...
        let mut engine = self.checkers.engine::<EFIModule>(
            &component,
            &project,
            self.symbols.clone(),
            self.types.clone(),
    ...
    fn should_analyse(&self, platform: &PlatformAttributes) -> bool {
        self.checkers.iter().any(|checker| {
            ((checker.platform() == EFIModule::NAME
                && platform.get_attr::<EFIComponentName>().is_some())
                || (checker.platform() == EFIStandalone::NAME
                    && platform.get_attr::<EFIStandaloneName>().is_some()))
                && EFIModule::should_check(checker.architecture(), checker.conditions(), platform)
                    .unwrap_or(false)
        })
    }
    ...
    fn analyse_and_prefilter(
        ...
        // TODO: see if we can somehow pass through the checks that are valid for this component
        // via `should_analyse` to avoid re-evaluating them here.

        let platform = component.container();
        for checker in self.checkers.iter() {
            if ((checker.platform() == EFIModule::NAME
                && platform.get_attr::<EFIComponentName>().is_some())
                || (checker.platform() == EFIStandalone::NAME
                    && platform.get_attr::<EFIStandaloneName>().is_some()))
                && EFIModule::should_check(checker.architecture(), checker.conditions(), &platform)
