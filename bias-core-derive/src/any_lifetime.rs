...
fn type_param_bound_replace_lifetimes_with_static(
    ...
    match bound {
        ...
        bound => Err(syn::Error::new_spanned(bound, "unsupported bound")),
    }
}
...
fn derive_provides_static_type_impl(
        ...
        match param {
            ...
            syn::GenericParam::Type(param) => {
                ...
                if has_static_lifetime_bound {
                    ...
                    type_param_bounds.push(quote! {
                        #param_name : #(#param_bounds+)* bias_core::gazebo::any::ProvidesStaticType + Sized
                    });
                    let param_bounds = param
                        .bounds
                        .iter()
    ...
    let gen = if input.generics.lt_token.is_none() {
        quote! {
            unsafe impl #impl_generics bias_core::gazebo::any::ProvidesStaticType for #name #ty_generics #where_clause {
        ...
        quote! {
            unsafe impl <
                ...
                #(#type_param_bounds,)*
                #(#const_params,)*
                    > bias_core::gazebo::any::ProvidesStaticType
            for #name <
                #(#lifetimes,)*
                #(#type_param_names,)*
                #(#const_param_names,)*
